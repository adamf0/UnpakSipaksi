...
{
    internal static class CreateDokumenPendukung
    {
        public static void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapPost("PenelitianHibah/DokumenPendukung", [IgnoreAntiforgeryToken(Order = 1001)] async ([FromForm] CreateDokumenPendukungRequest request, ISender sender, IFileProvider fileProvider, HttpContext context, TokenValidator tokenValidator) =>
            {
                ...
                Result<Guid> result = await sender.Send(new CreateDokumenPendukungCommand(
              ...
              .Accepts<CreateDokumenPendukungRequest>("multipart/form-data")
              .DisableAntiforgery();
        }

        internal sealed class CreateDokumenPendukungRequest
        {
