...
using UnpakSipaksi.Modules.Insentif.Application.CreateInsentifJurnal;
using UnpakSipaksi.Modules.Insentif.Application.CreateInsentifProsiding;

namespace UnpakSipaksi.Modules.Insentif.Presentation.Insentif
{
    internal static class CreateInsentif
    {
        public static void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapPost("Insentif", async (CreateInsentifRequest request, ISender sender) =>
            {
                if (request.JenisPublikasi.ToLower() == Constant.jurnalKey)
                {
                    // Handle external type insentif (Jurnal)
                    Result result = await sender.Send(new CreateInsentifJurnalCommand(
                        request.Nidn,
                ...
                else if (request.JenisPublikasi.ToLower() == Constant.prosidingKey)
                {
                    ...
                    Result result = await sender.Send(new CreateInsentifProsidingCommand(
                        request.Nidn,
                        request.JudulArtikel,
                        request.NamaJurnalPenerbit,
                        request.JumlahPenulis,
        ...
        internal sealed class CreateInsentifRequest
        {
