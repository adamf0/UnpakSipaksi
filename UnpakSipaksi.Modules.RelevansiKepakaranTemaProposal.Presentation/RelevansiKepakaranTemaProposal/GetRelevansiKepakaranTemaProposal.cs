...
{
    internal static class GetRelevansiKepakaranTemaProposal
    {
        public static void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet("RelevansiKepakaranTemaProposal/{id}", async (Guid id, ISender sender) =>
            {
                Result<RelevansiKepakaranTemaProposalResponse> result = await sender.Send(new GetRelevansiKepakaranTemaProposalQuery(id));
