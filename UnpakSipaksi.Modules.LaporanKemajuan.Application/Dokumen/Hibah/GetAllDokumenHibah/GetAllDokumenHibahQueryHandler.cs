...
{
    internal sealed class GetAllDokumenHibahQueryHandler(IDbConnectionFactory _dbConnectionFactory) : IQueryHandler<GetAllDokumenHibahQuery, List<DokumenHibahResponse>>
    {
        public async Task<Result<List<DokumenHibahResponse>>> Handle(GetAllDokumenHibahQuery request, CancellationToken cancellationToken)
        {
            ...
            var result = await connection.QueryAsync<DokumenHibahResponse>(sql, new { UuidPenelitianHibah = request.UuidPenelitianHibah });

            if (result == null || !result.Any())
            {
                return Result.Failure<List<DokumenHibahResponse>>(DokumenErrors.EmptyData());
            }
