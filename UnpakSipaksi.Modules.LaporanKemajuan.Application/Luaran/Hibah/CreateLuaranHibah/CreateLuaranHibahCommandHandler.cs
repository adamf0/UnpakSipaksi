...
{
    internal sealed class CreateLuaranHibahCommandHandler(
    IDokumenHibahRepository repository,
    IPenelitianHibahApi penelitianHibahApi,
    IUnitOfWorkDokumenHibah unitOfWork)
    : ICommandHandler<CreateLuaranHibahCommand, Guid>
    {
        public async Task<Result<Guid>> Handle(CreateLuaranHibahCommand request, CancellationToken cancellationToken)
        {
