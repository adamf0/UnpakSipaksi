...
public sealed class KetajamanAnalisisApiTests
{
    private readonly Mock<ISender> _senderMock;
    private readonly KetajamanAnalisisApi _api;

    public KetajamanAnalisisApiTests()
    {
        _senderMock = new Mock<ISender>();
        _api = new KetajamanAnalisisApi(_senderMock.Object);
    }
    ...
    public async Task GetAsync_ShouldReturnNull_WhenFailure()
    {
        ...
        _senderMock
            .Setup(s => s.Send(It.IsAny<GetKetajamanAnalisisDefaultQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Failure<KetajamanAnalisisDefaultResponse>(
                KetajamanAnalisisErrors.NotFound(uuid)
            ));
    ...
    public async Task GetAsync_ShouldReturnMappedResponse_WhenSuccess()
    {
        ...
        var response = new KetajamanAnalisisDefaultResponse
        {
            Id = "12",
            Uuid = uuid.ToString(),
            Nama = "Artikel Jurnal",
    ...
    public async Task GetBobotWithoutTargetAsync_ShouldReturnNull_WhenFailure()
    {
        // Arrange
        _senderMock
            .Setup(s => s.Send(It.IsAny<GetBobotKetajamanAnalisisQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Failure<int?>(
                KetajamanAnalisisErrors.UnknownKategoriSkema()
            ));
