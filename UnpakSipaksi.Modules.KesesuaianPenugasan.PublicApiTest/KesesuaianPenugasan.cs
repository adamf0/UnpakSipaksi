...
public sealed class KesesuaianPenugasanApiTests
{
    private readonly Mock<ISender> _senderMock;
    private readonly KesesuaianPenugasanApi _api;

    public KesesuaianPenugasanApiTests()
    {
        _senderMock = new Mock<ISender>();
        _api = new KesesuaianPenugasanApi(_senderMock.Object);
    }
    ...
    public async Task GetAsync_ShouldReturnNull_WhenFailure()
    {
        ...
        _senderMock
            .Setup(s => s.Send(It.IsAny<GetKesesuaianPenugasanDefaultQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Failure<KesesuaianPenugasanDefaultResponse>(
                KesesuaianPenugasanErrors.NotFound(uuid)
            ));
    ...
    public async Task GetAsync_ShouldReturnMappedResponse_WhenSuccess()
    {
        ...
        var response = new KesesuaianPenugasanDefaultResponse
        {
            Id = "12",
            Uuid = uuid.ToString(),
            Nama = "Artikel Jurnal",
    ...
    public async Task GetBobotWithoutTargetAsync_ShouldReturnNull_WhenFailure()
    {
        // Arrange
        _senderMock
            .Setup(s => s.Send(It.IsAny<GetBobotKesesuaianPenugasanQuery>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Failure<int?>(
                KesesuaianPenugasanErrors.UnknownKategoriSkema()
            ));
