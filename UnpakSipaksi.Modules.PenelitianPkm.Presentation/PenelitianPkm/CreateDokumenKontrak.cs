...
{
    internal static class CreateDokumenLainnya
    {
        public static void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapPost("PenelitianPkm/DokumenLainnya", [IgnoreAntiforgeryToken(Order = 1001)] async ([FromForm] CreateDokumenLainnyaRequest request, ISender sender, IFileProvider fileProvider, HttpContext context, TokenValidator tokenValidator) =>
            {
                string? dokumenPath = null;
                var uploadsFolder = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "uploads/pkm/dokumen_lainnya");
                ...
                if (request.File != null && request.File.Length > 0)
                {
                    ...
                    dokumenPath = "pkm/dokumen_lainnya/" + safeFileName; // Relative path
                }

                Result<Guid> result = await sender.Send(new CreateDokumenLainnyaCommand(
              ...
              .Accepts<CreateDokumenLainnyaRequest>("multipart/form-data")
              .DisableAntiforgery();
        }

        internal sealed class CreateDokumenLainnyaRequest
        {
            [FromForm] public string UuidPenelitianPkm { get; set; }
            [FromForm] public string? Keterangan { get; set; }
