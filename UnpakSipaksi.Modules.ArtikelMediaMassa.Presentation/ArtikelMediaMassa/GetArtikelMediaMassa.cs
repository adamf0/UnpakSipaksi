...
{
    internal static class GetArtikelMediaMassa
    {
        public static void MapEndpoint(IEndpointRouteBuilder app)
        {
            app.MapGet("ArtikelMediaMassa/{id}", async (Guid id, ISender sender) =>
            {
                Result<ArtikelMediaMassaResponse> result = await sender.Send(new GetArtikelMediaMassaQuery(id));
